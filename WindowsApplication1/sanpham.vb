...
Public Class frmsanpham
    ...
    Private Function themsp() As Boolean
        Dim lenhsql As String = "INSERT INTO SANPHAM(MaSP, MaLoai, TenSP, Gia, NgaySX)"
        lenhsql += String.Format("VALUES('{0}','{1}','{2}','{3}','{4}')",
                                 txtmasp.Text, txtmaloai.Text, txttensp.Text, txtgia.Text, txtngay.Text)
        Return DBAccess.ExecuteNoneQuery(lenhsql)
    End Function
    'dinh nghia ham kiem tra gia tri truoc khi insert
    Private Function rong() As Boolean
        Return (String.IsNullOrEmpty(txtmasp.Text) OrElse String.IsNullOrEmpty(txtmaloai.Text) OrElse
            String.IsNullOrEmpty(txttensp.Text) OrElse String.IsNullOrEmpty(txtgia.Text) OrElse String.IsNullOrEmpty(txtngay.Text))
    End Function
    Public Function Loadsanpham() As DataSet
        Dim chuoiketnoi As String = "Data Source=DESKTOP-JUIK989\SQLEXPRESS;Initial Catalog=QLBANHANG;Integrated Security=True"
        Dim conn As SqlConnection = New SqlConnection(chuoiketnoi)
        Dim LoadSP As New SqlDataAdapter("select MaSP as 'Mã sản phẩm',  MaLoai as 'Mã Loại', TenSP as 'Tên sản phẩm',  Gia as 'Gía Tiền', NgaySX as 'Ngày SX' from SANPHAM ", conn)
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        Dim sqlquery As String = String.Format("select MaSP as 'Mã sản phẩm', MaLoai as 'Mã Loại', TenSP as 'Tên sản phẩm', Gia as 'Gía Tiền', NgaySX as 'Ngày SX' from SANPHAM ")
    ...
    Private Sub dgvsp_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvsp.CellContentClick
        ...
        txtmaloai.Text = dgvsp.Item(1, click).Value
        txttensp.Text = dgvsp.Item(2, click).Value
        txtgia.Text = dgvsp.Item(3, click).Value
        txtngay.Text = dgvsp.Item(4, click).Value
    End Sub

    Private Sub btnedit_Click(sender As Object, e As EventArgs) Handles btnedit.Click
        Dim conn As SqlConnection = New SqlConnection(chuoiketnoi)
        Dim updatequery As String = "update SANPHAM set MaSP=@MASP, MaLoai=@MALOAI, TenSP=@TENSP, Gia=@GIA,  NgaySX=@NGAYSX where MASP=@MASP"
                    ...
                    Else
                        ...
                        Else
                            ...
                            Else
                                ...
                                addupdate.Parameters.AddWithValue("@NGAYSX", txtngay.Text)
